...
from app.origin_guard import enforce_same_origin
from app.rate_limit import enforce_run_rate_limit
from app.schemas.run import Run, RunCreate, RunWithSteps

router = APIRouter(prefix="/runs", tags=["runs"])

_guard_dependencies = [Depends(enforce_same_origin), Depends(enforce_run_rate_limit)]
...
@router.post("", response_model=Run, status_code=201, dependencies=_guard_dependencies)
...
@router.post("/{run_id}/fork", response_model=Run, status_code=201, dependencies=_guard_dependencies)
